...
def get_info_from_db(obs_id):
	...
	cursor.execute(
		"select o.tstartreal,o.tendreal-o.tstartreal as duration,o.lon,o.lat,ins.irf,ins.caldb,ins.fov,t.emin,t.emax from observation o join observation_target ot ON (ot.observationid = o.observationid) JOIN target t on(t.targetid = ot.targetid) join observingmode om ON (om.observingmodeid = t.observingmodeid) join instrument ins on (ins.instrumentid = om.instrumentid) where o.observationid = " + obs_id)
